...
class CRUDWorkflow(CRUDBase[Workflow, WorkflowCreateSch, WorkflowUpdateSch]):
    ...
    async def notification(self, payload: dict, request:Request):

        sch = WorkflowSystemCallbackSch(**payload)

        obj_current = await self.get_by_reference_id(reference_id=sch.client_reff_no)
        ...
        obj_new = WorkflowUpdateSch(
            reference_id=obj_current.reference_id,
            txn_id=sch.txn_id,
    ...
    async def handle_completed(self, workflow: Workflow, request: Request):
        match workflow.entity:
            case WorkflowEntityEnum.MEMO:
                await self.memo_completed(workflow=workflow, request=request)

    async def memo_completed(self, workflow: Workflow, request: Request):
        url = f'{request.base_url}arsip/memo/task'
        GCloudTaskService().create_task(payload={"id":str(workflow.reference_id)}, base_url=url)
